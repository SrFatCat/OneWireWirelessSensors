...
uint8_t RCSwitch::oregonTimingsCount = 0;
unsigned int RCSwitch::oregonTimings[RC_MAX_PULSE_BUFFER];
...
void RECEIVE_ATTR RCSwitch::handleInterrupt() {
  ...
  if (oregonTimingsCount < RC_MAX_PULSE_BUFFER /*&& digitalRead(2) == LOW*/) {
	  oregonTimings[oregonTimingsCount++] = duration;
  }
...
byte OregonDecoder::SumCRC()
{
	int count = (pos > 8) ? 8 : ((pos > 6) ? 6 : 0);
	int s = 0;
...
{
	return ((data[5] & 0xF0) >> 4) * 10 + (data[5] & 0xF) + (float)(((data[4] & 0xF0) >> 4) / 10.0) * ((data[6] & 0x8) ? -1. : 1.);
}

float OregonDecoder::getHumidity()
{
	return  (data[7] & 0xF) * 10 + ((data[6] & 0xF0) >> 4) + ((data[7] & 0xF0) >> 4) / 10.0;
}

byte OregonDecoder::getChannel()
{
	byte channel;
	switch (data[2] >> 4)
	{
...
bool OregonDecoder::okCRC() {
	bool ret = false;
	if (pos>8) {
		ret =  (data[8] == (SumCRC() - 0xa) & 0xFF);
	}
	else if (pos > 6) {
		int sum = ((SumCRC() + (data[6] & 0xF) - 0xa) & 0xff);
		ret = ((sum & 0xF) == (data[6] >> 4) && (sum >> 4) == (data[7] & 0xF));
	}

	if (!(complete = ret)) resetDecoder();
	return ret;
}
...
char OregonDecoderV3::decode(word width)
{
	if (100 <= width && width < 1200) {
		...
		if (width >= 1500 && pos >= 8) return 1; else return -1;
	}
	return 0;
}

void WH2Decoder::gotBit(char value)
{
...
char WH2Decoder::decode(word width)
{
	if (200 <= width && width < 2000) {
		if (width >= 600 && width <= 1200) return 0; // pause
		byte w = width >= 1000;
		switch (state) {
		case UNKNOWN:
			if (w==0) //HiBit
		...
		//if (state == OK) Serial.println(" " + String(width) + " tb:" + total_bits);
		return -1;
	}
	if (pos == 6) return 1; else return 0;
}
